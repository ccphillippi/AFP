...
class AntonymReplacer(object):
    delimiter=['and','or','when','whenever','whether','because','unless','until','since','hence','so','as','which'
               ,'who','why','where','for','like','?','.',',']
    def replace(self, word, pos=None):
        ...
        if len(antonyms) > 0:
            ...
            return "not_"+word
    def replace_negations_tree(self,tree,bool_not=False):
        for i in xrange(0,len(tree)):
            #child=tree[i]
            if type(tree[i]) is not tuple:
                self.replace_negations_tree(tree[i],bool_not)
            elif bool_not and tree[i][0] in self.delimiter:
                bool_not=False
            elif bool_not:
                tree[i]=(self.replace(tree[i][0]),tree[i][1])
            elif tree[i][0].lower()=='not':
                bool_not=True
    def replace_negations_pos(self,sent):
        bool_not=False
        for i in xrange(0,len(sent)):
            if bool_not and sent[i][0] in self.delimiter:
                bool_not=False
            elif bool_not:
                sent[i]=(self.replace(sent[i][0]),sent[i][1])
            elif sent[i][0].lower()=='not':
                bool_not=True
                         
    def FindSubTree(self, tree, str1,str2):
        for t in tree:
            if type(t) is nltk.tree.Tree: 
                if str1 in zip(*t.flatten().leaves())[0] and str2 in zip(*t.flatten().leaves())[0]:
                    return self.FindSubTree(t, str1,str2)
        if (str1 in zip(*tree.flatten().leaves())[0]) and (str2 in zip(*tree.flatten().leaves())[0]):
            return tree
        else:
            return None
